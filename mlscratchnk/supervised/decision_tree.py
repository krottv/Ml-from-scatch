...
class TreeNode():
    ...
    def displayName(self, label_classes = None, label_features = None):
        ...
        if self.depth == 0:
            return 'root node {:} - {:}, nsamples {:}'.format(displayFeature, self.feature_threshold, self.impurity, self.nsamples)
        elif self.is_leaf():
...
class DecisionTree(ABC):
    ...
    def __init__(self, max_depth=32, min_samples_split=2, min_samples_leaf=1, debug=False, 
...
class DecisionTreeRegressor(DecisionTree):
    ...
    criterion (default mse)
        mse
            Mean squared error, which is equal to variance reduction

    """

    def __init__(self, max_depth=32, min_samples_split=2, min_samples_leaf=1, debug=False, 
    splitter='best', min_impurity_decrease = 0, random_state=None, criterion = 'mse'):
        super().__init__(max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, debug=debug, splitter=splitter, min_impurity_decrease=min_impurity_decrease, random_state=random_state)
    ...
    def calculate_impurity(self, y, y_left, y_right):
        ...
        percent_left = len(y_left) / n
        percent_right = len(y_right) / n

        if self.criterion == 'mse':
            return (self.mse_criterion(y_left) * percent_left, self.mse_criterion(y_right) * percent_right)
            
        else:
            raise Exception(f'illegal criterion {self.criterion}')
...
class DecisionTreeClassifier(DecisionTree):
    ...
    criterion (default gini)
        gini
            probability that we classify datapoints incorrectly

            where p is probability for each class
            gini = sum(p * (1 - p)) 
        ...
        entropy
            ...
            basically it is very similar to gini and gives very similar results. Gini is preferred because it is faster to compute.
        
    """
    
    def __init__(self, max_depth=32, min_samples_split=2, min_samples_leaf=1, debug=False, 
    splitter='best', min_impurity_decrease = 0, random_state=None, criterion = 'gini'):
        self.criterion = criterion
        super().__init__(max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, debug=debug, splitter=splitter, min_impurity_decrease=min_impurity_decrease, random_state=random_state)
